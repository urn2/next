...
class hImage_Verify{
	...
	public function Font($Filename)
	{
		...
		$this->CharArea =imagettfbbox($this->FontSize, 0, $this->Font, '0');
		return $this;
	}
	/**
	 * 调整图片尺寸
	...
	public function DrawQuestion($MaxNum =10)
	{
		...
		$_SESSION[Next::Config('valid', 'value')] =$Info['r'];
		$_SESSION[Next::Config('valid', 'time')] =time();
		$Code =$Info['m'];
		$CodeNum =strlen($Code);
		for ($i=0;$i<strlen($Code);$i++){
			...
			if (empty($this->Font) || !is_file($this->Font)) {
				...
				$_angle =mt_rand(0,1) ?mt_rand(330,360) :mt_rand(0,30);
				imagettftext($this->Bmp, $this->FontSize, $_angle, 
				$i*$this->Width/$CodeNum+mt_rand(1,4)-$this->CharArea[6],
				mt_rand(1,4)-$this->CharArea[7], 
				$c, $this->Font, $Code[$i]);
			}
    	}
		return $this;
	 ...
	 * @return object 对象本身
	 */
	static public function MakeCode($Length)
	{
		$r ='';
	 ...
	 * @return object 对象本身
	 */
	private function MakeChar($Length)
	{
	 ...
	 * @return object 对象本身
	 */
	public function DrawChar($CodeNum =4, $MatchCase=false)
	{
		$Code =self::MakeChar($CodeNum);
		$_SESSION[Next::Config('valid', 'value')] =$MatchCase ?$Code :strtolower($Code);
		$_SESSION[Next::Config('valid', 'time')] =time();
		for ($i=0;$i<strlen($Code);$i++){
			$r =self::ClearColor($this->Background[0]);
			$g =self::ClearColor($this->Background[1]);
