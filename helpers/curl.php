...
class hCURL{
	...
	public function parse(){
		...
		$r = $this->RW();
		//vFL::out($this->opts[CURLOPT_URL], $r);
		$ra = json_decode($r, true);
		//vFL::out($ra);
		if(is_array($ra) && (isset($ra['data']) || isset($ra['err']) || isset($ra['msg']))) return $ra;
		return false;
		return ['err' => -1, 'msg' => 'service err.'];
	}
}