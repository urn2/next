...
class hSQL{
	 ...
	 * @param string $Config 配置名，省却读取应用默认default配置
	 * @return hSQL
	 */
	...
	public function _clone(){
		$clone =self::factory($this->table, $this->primary, $this->_db_cfg);
		$clone->args =$this->args;
		return $clone;
	}
	public function clear(){
		return self::factory($this->table, $this->primary, $this->_db_cfg);
	}
	...
	public function read($SQL = "", $Clear =true){
		...
		if(is_object($r)) return $r->fetch_all(MYSQLI_ASSOC);//SQLITE3_ASSOC, MYSQLI_ASSOC,
		return [];
	}
	/**
	 * 返回第一条记录
	...
	/**
	 * 继承 select, filter
	 * join(hSQL, ['id'])->join('user', ['id'=>'user_id'])->join('user', ['user.id'=>'editor.user_id'])
	 *
	 * @param string|hSQL $Table //表名
	...
	private function _db(){
		if(!isset(self::$db[$this->_db_cfg])) self::$db[$this->_db_cfg] = hDB::factory($this->_db_cfg);
		return self::$db[$this->_db_cfg];
	}
	/**
	 * 返回最后数据库错误
	 */
